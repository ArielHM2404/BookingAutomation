...
export async function extractDates(page: Page) {
    ...
    const departureDateString = await page.locator('span.aJ3v').first().textContent();
    // Extract return date
    ...
    const departureDate = new Date(departureDateString.replace(/^(\w{3}) (\d{1,2})/, '$2 $1'));
    const returnDate = new Date(returnDateString.replace(/^(\w{3}) (\d{1,2})/, '$2 $1'));
...
export function parseDate(dateStr: string): Date {
  ...
  export function verifyDepartureBeforeReturn(departureDateStr: string, returnDateStr: string): void {
    ...
    if (isNaN(departureDate.getTime()) || isNaN(returnDate.getTime())) {
      throw new Error('Invalid date format: Unable to parse the dates correctly.');
    }
